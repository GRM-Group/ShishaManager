...
public class SMLogger {
	...
	public static void initLogger() {
		...
		try {
			if (!mainDir.exists()) {
				if (!mainDir.mkdir()) {
					throw new IOException("Cannot create main app folder!");
				}
			}
	...
	public static void printDebugFieldValue(Object obj,
			String... stringsFieldNames) {
		...
		while (sClazz.getSuperclass() != null
				&& sClazz.getSuperclass() != Object.class) {
				...
				for (Field field : fields) {
					if (field.getName().equalsIgnoreCase(string)) {
						if (clazz.getMethod("toString") != null) {
							...
							System.out.print(
									string + ": " + method.invoke(objCasted));
						} else {
